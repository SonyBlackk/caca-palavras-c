...
// Funcoes recursivas (exemplo, a ser implementada conforme necessidade)
int buscar_palavra_recursiva(char** matriz, int linhas, int colunas, const char* palavra);
...
// Funcao wrapper para iniciar a busca recursiva
int buscar_palavra_recursiva(char** matriz, int linhas, int colunas, const char* palavra) {
    for (int r = 0; r < linhas; r++) {
        for (int c = 0; c < colunas; c++) {
            if (buscar_palavra_recursiva_aux(matriz, linhas, colunas, palavra, r, c, 0)) {
                return 1; // Palavra encontrada na matriz
            }
        }
    }
